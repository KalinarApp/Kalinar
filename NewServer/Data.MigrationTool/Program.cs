...
async Task ExecuteAction(string actionName, Func<Task> action)
{
    Console.WriteLine($"Start migrating {actionName}.");

    await action();

    Console.WriteLine($"Finished migrating {actionName}");
}
...
await ExecuteAction("Users", async () => {
...
await ExecuteAction("Groups", async () =>
{
    ...
    foreach (Group group in await heroContext.Groups.Include(group => group.Members).ToListAsync())
    {
        ...
        IEnumerable<Attribute> oldDefaultAttributes = await heroContext.Attributes.Where(item => item.GroupId == Guid.Empty).ToListAsync();
        ...
        defaultAttributes.ToList().ForEach(item => migratedAttributes.Add(oldDefaultAttributes.First(old => old.Name == item.Name).Id, item));
...
await ExecuteAction("Abilities", async () =>
{
...
// Attributes
await ExecuteAction("Attributes", async () =>
{
...
await ExecuteAction("Skills", async () =>
{
...
await ExecuteAction("Races", async () =>
{
...
await ExecuteAction("Characters", async () =>
{
...
await ExecuteAction("Skilltrees", async () =>
{
...
await ExecuteAction("Skilltree Nodes", async () =>
{
...
await ExecuteAction("Skilltree Edges", async () =>
{
    foreach (SkilltreeNode node in await heroContext.SkilltreeNodes.ToListAsync())
    {
        if (!migratedSkilltrees.ContainsKey(node.ParentId!.Value)) continue;
...
// Story events
await ExecuteAction("Story Events", async () =>
{
