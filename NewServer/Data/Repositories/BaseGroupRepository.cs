...
namespace Kalinar.Data.Repositories
{
        ...
        where T : BaseEntityWithGroup
    {
        ...
        {
            return await this.context.Set<T>().Include(item => item.Group).ThenInclude(item => item.Members).Where(item => item.GroupId == groupId).ToListAsync(cancellationToken);
        }

        public override async Task<T?> FindByIdAsync(Guid id, CancellationToken cancellationToken = default)
        {
            return await this.context.Set<T>().Include(item => item.Group).ThenInclude(item => item.Members).FirstOrDefaultAsync(item => item.Id == id, cancellationToken);
        }
    }
}
