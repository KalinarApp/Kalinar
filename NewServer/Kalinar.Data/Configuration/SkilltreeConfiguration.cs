...
namespace Kalinar.Data.Configuration
{
    internal class SkilltreeConfiguration : IEntityTypeConfiguration<SkilltreeEntity>
    {
        public void Configure(EntityTypeBuilder<SkilltreeEntity> builder)
        {
            ...
            //builder.HasMany(x => x.Nodes).WithOne().HasForeignKey(x => x.SkilltreeId).OnDelete(DeleteBehavior.Cascade);
            //builder.HasMany(x => x.Edges).WithOne().HasForeignKey(x => x.SkilltreeId).OnDelete(DeleteBehavior.Cascade);
        }
    }
}
